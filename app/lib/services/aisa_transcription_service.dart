  ...
  Future<String?> _transcribe(File wavFile, {String? previousContext, bool skipClaude = false}) async {
    ...
    if (await _isSilentWav(wavFile)) {
      AisaDebugLogger.instance.vad(
        '無音検出 → Groq APIスキップ (${(fileSize / 1024).toStringAsFixed(0)}KB)',
        context: {
          'fileSizeKB': (fileSize / 1024).toStringAsFixed(0),
          'reason': 'silentWav',
        },
      );
    ...
    AisaDebugLogger.instance.groq(
      'Groq API送信: ${(fileSize / 1024).toStringAsFixed(0)}KB'
          '${previousContext != null ? ", 文脈あり(${previousContext.length}chars)" : ""}',
      context: {
        'fileSizeKB': (fileSize / 1024).toStringAsFixed(0),
        'hasContext': previousContext != null,
        'contextChars': previousContext?.length ?? 0,
      },
    );
    ...
    final groqStartedAt = DateTime.now();
    final streamedResponse = await request.send().timeout(
      const Duration(seconds: 90),
      onTimeout: () => throw TimeoutException('Groq API request timed out after 90s'),
    );
    final body = await streamedResponse.stream.bytesToString();
    final groqElapsedMs = DateTime.now().difference(groqStartedAt).inMilliseconds;

    if (streamedResponse.statusCode == 429) {
      AisaDebugLogger.instance.error(
        '❌ Groq レートリミット(429) - 60秒後にリトライ',
        category: AisaLogCategory.groq,
        context: {
          'status': 429,
          'elapsedMs': groqElapsedMs,
          'fileSizeKB': (fileSize / 1024).toStringAsFixed(0),
          'bodySnippet': body.substring(0, body.length.clamp(0, 120)),
        },
      );
    ...
    if (streamedResponse.statusCode != 200) {
      AisaDebugLogger.instance.error(
        '❌ Groq APIエラー ${streamedResponse.statusCode}: ${body.substring(0, body.length.clamp(0, 100))}',
        category: AisaLogCategory.groq,
        context: {
    ...
    AisaDebugLogger.instance.groq(
      'Groq APIレスポンス: HTTP ${streamedResponse.statusCode} OK (${groqElapsedMs}ms)',
      context: {
        'status': streamedResponse.statusCode,
        'elapsedMs': groqElapsedMs,
...
''';
      ...
      final claudeStartedAt = DateTime.now();
      ...
      final claudeElapsedMs = DateTime.now().difference(claudeStartedAt).inMilliseconds;

      if (response.statusCode != 200) {
        AisaDebugLogger.instance.error(
          '❌ Claude API ${response.statusCode}',
          category: AisaLogCategory.claude,
          context: {
            'status': response.statusCode,
            'elapsedMs': claudeElapsedMs,
            'bodySnippet': response.body.substring(0, response.body.length.clamp(0, 120)),
          },
        );
      ...
      if (result != whisperText) {
        AisaDebugLogger.instance.claude(
          'Claude校正: 変更あり (${whisperText.length}→${result.length}文字, ${claudeElapsedMs}ms)',
          context: {
            'beforeChars': whisperText.length,
            'afterChars': result.length,
            'deltaChars': result.length - whisperText.length,
            'elapsedMs': claudeElapsedMs,
          },
        );
        ...
        AisaDebugLogger.instance.claude(
          'Claude校正: 変更なし (${result.length}文字, ${claudeElapsedMs}ms)',
          context: {
            'chars': result.length,
            'elapsedMs': claudeElapsedMs,
          },
